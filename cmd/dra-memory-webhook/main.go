...
var validProfiles = map[string]profiles.ConfigHandler{
	...
	memorythroughput.ProfileName: memorythroughput.Profile{},
}

func defaultDriverName(profile string) string {
	switch profile {
	case memorythroughput.ProfileName:
		return "mem.example.com"
	default:
...
func newApp() *cli.App {
	...
	app := &cli.App{
		Name:            "dra-memory-webhook",
		Usage:           "dra-memory-webhook implements a validating admission webhook complementing a DRA driver plugin.",
		ArgsUsage:       " ",
		...
		Action: func(c *cli.Context) error {
			...
			if flags.driverName == "" {
				flags.driverName = defaultDriverName(flags.profile)
			}
